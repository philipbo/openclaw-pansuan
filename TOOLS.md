...
- **第二行数据（客）**：第 2～11 列 → 0:1、0:2、1:2、0:3、1:3、2:3、0:4、1:4、2:4、3:4。

**提取步骤**：在 snapshot/HTML 中定位第一格为「波胆」的 `<table>`（或含 `class="bui"` 且首格为波胆的 tr）→ 取下一行（主）每个 `<td>` 的文本，按上表对应到比分；再取再下一行（客）前 10 个 `<td>` 对应到 0:1～3:4 → 拼成「比分→赔率」映射。
