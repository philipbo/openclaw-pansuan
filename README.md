...
| 深度分析 | 5 维度分析（基本面、伤停、交锋、盘赔、战意） | 自动(Cron) / 手动 |
| 推荐输出 | 生成结构化推荐卡片并推送                     | 自动(Cron) / 手动 |
...
- `agents.pansuan.cron` — 定时任务（11:30 早波赛前分析，17:00 晚波赛前分析，09:30 赛后复盘）
...
- **每天 11:30（早波）**：抓取赛程 → 初筛 → 深度分析 → 推荐（重点亚洲联赛，12:00-14:00 开球）
- **每天 17:00（晚波）**：补充抓取 → 初筛 → 完整深度分析 → 推荐（重点欧洲联赛，18:00+ 开球）
...
| 「今天有什么比赛」         | 抓取并展示今日赛程     |
| 「帮我筛一下」             | 执行初筛流程           |
| 「分析一下阿森纳 vs 曼城」 | 对指定比赛做深度分析   |
| 「给推荐」「今天买什么」   | 执行完整赛前流程并推荐 |
| 「昨天结果怎么样」         | 复盘昨日推荐           |
| 「战绩怎么样」             | 展示累计统计           |
| 「只看英超」               | 调整筛选范围           |
| 「多推几场」               | 放宽推荐数量           |
...
- 「以后最多推 3 场」→ 调整 RECOMMENDED_DAILY
- 「英冠也要看」→ 调整联赛层级
- 「推送时间改到 15:00」→ 调整 Cron 配置
