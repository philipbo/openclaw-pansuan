...
**当深度分析场次为 0 时**（初筛通过 0 场）：跳过步骤 1–3，直接生成一条汇总消息「今日无候选场次，无推荐」，推送并写入 `## 推荐`（精选 0 场、无串关），然后结束。**写入的 `## 推荐` 仍按步骤 5 的标准结构**（含 ### 精选场次、### 候补场次、### 串关组合、### CLV 追踪数据），表为空即可，便于 post-review 解析一致。
...
🎯 推荐：
  ...
  📏 大小球：{大/小 X.5 球}（盘口：{X.5}，预期总进球：{λ1+λ2}）｜或 ⚠️ 观望
...
| ---- | ------ | ---- | ------ | ---- | ------ | -------- | ------ | ---- | -------- |
| {主队} vs {客队} | {matchId} | {HH:MM} | {主胜/平/客胜} | {让X 上/下/观望} | {大/小X.5/观望} | {比分1} / {比分2} / {比分3} | {高/中/低} | {🔒/✅} | {理由} |
