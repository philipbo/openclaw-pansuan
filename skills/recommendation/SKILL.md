...
来自 deep-analysis 的每场比赛综合评估结果。**执行 recommendation 时**，以收齐的 announce 所包含的**综合评估与 memory 摘要**为输入，**无需**再从 memory 读取 `## 深度分析`。
...
**当深度分析场次为 0 时**（初筛通过 0 场）：跳过步骤 1–3，直接生成一条汇总消息「今日无候选场次，无推荐」，推送并写入 `## 推荐`（精选 0 场、无串关），然后结束。推送消息可带一句「⏳ 今日购彩截止：{22:00/23:00}」以与正常汇总一致。**写入的 `## 推荐` 仍按步骤 5 的标准结构**（含 ### 精选场次、### 候补场次、### 串关组合）。精选/候补表无数据行。
...
🎯 推荐：
  ⚽ 胜平负：首选 {主胜/平局/客胜}；次选 {主胜/平局/客胜}
    ...
    └ 赔率依据：{首选/次选依据各一句}
  🏈 亚盘：{让球方向}（盘口：{X}）└ 即时：主水 {X.XX} 客水 {X.XX}
...
- 亚盘：「主队让{X}球 主胜」「主队受让{X}球 主胜」；方向不明确时标「⚠️ 观望」
- 大小球：改为 **首选 + 次选** 两档盘口：`首选 大/小 X 球；次选 大/小 Y 球`（盘口：X/Y，预期总进球：λ1+λ2）。次选用于覆盖/风控；方向不明确或与泊松比分重大矛盾时标「⚠️观望」。
- 比分：「比分 X:X」
- **两套比分（deep-analysis 矛盾场次）**：当 deep-analysis 输出了两套比分（整体 Top 3 + 与大小球同方向 Top 3）时，recommendation 的汇总表「比分」列默认填整体 Top 3；同时在该场的「核心矛盾」或「理由」中补一句「本场存在大小球 vs 泊松比分矛盾，另有同方向 Top 3 参考」。
...
**比分口径（强制统一）**：推荐输出与复盘统计的泊松比分最多 **6 个**。无矛盾时可写 Top 4～6；矛盾时两套各 Top 3。不得输出完整概率表。
...
**同一场多次推荐**：复盘只以该最后一段内的推荐为准；同一场比赛若曾在当日更早的推荐段中出现，不合并、不追溯，仅本段有效。写入新推荐段时，若某场（matchId）在当日更早的推荐段中已出现过，可在该场精选理由后加注「（当日曾于 HH:MM 推荐，本段为最新）」便于龙王对比，不影响复盘逻辑。
...
- 比分推荐也记录，用于复盘比分命中率
- **命中率统计主体 = 精选场次**（龙王实际投注的）
- 候补场次结果也在复盘中展示（作为参考：如果选了会怎样），但不计入主要统计
- **同一场在当日多段推荐中**：复盘只认**最后一段**内该场的推荐，不合并、不追溯更早段
