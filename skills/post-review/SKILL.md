...
```
CLV% = (推荐时赔率 / 收盘赔率 - 1) × 100

正值 = 推荐时拿到了比收盘更高的赔率（你比市场更早更准）→ 正 CLV ✅
负值 = 推荐时赔率低于收盘（市场比你更准）→ 负 CLV ❌

示例：推荐时赔率 2.10，平博收盘 1.95 → (2.10/1.95 - 1) × 100 = +7.7% ✅
示例：推荐时赔率 1.80，平博收盘 1.90 → (1.80/1.90 - 1) × 100 = -5.3% ❌
```
