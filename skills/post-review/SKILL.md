...
- 胜平负推荐 → `1x2`（取主要参考公司，如威廉/365）
- 让球推荐 → `handicap`（取主要参考公司，如澳彩/皇冠）
- 大小球推荐 → `overunder`（取主要参考公司，如澳彩/皇冠）
...
- 比分进球数正确数 += 当日进球数正确数
- 累计比分精准率 = 精准命中数 / 总推荐数
- CLV 统计：正 CLV 场次数、负 CLV 场次数、平均 CLV%
...
📈 当日 CLV：平均 {+/-X.X%}（正CLV {N}场 / 负CLV {N}场）
📈 累计战绩：推荐 {N} 场，命中 {N} 场，命中率 {XX}%
📈 累计比分：精准命中 {N}/{总N}（{XX}%）
📈 累计 CLV：平均 {+/-X.X%}（正CLV {N}场 / 负CLV {N}场）
```
