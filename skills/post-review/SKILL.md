...
- **定位方式**：在 `## 深度分析` 中按该场 **matchId**（摘要块内含 `比赛ID: {matchId}`）或 **联赛 + 主队 + 客队** 匹配到对应的 `### {联赛} {主队} vs {客队}（...）` 摘要块，再读取其下「综合评估」「泊松完整概率表」等子块。若某场在 ## 深度分析 中无对应块（如补复盘且原日未存摘要），该场 4a 中泊松 EV/锚定等写「无摘要」或省略。
...
- **亚盘/大小球泊松 EV**（若摘要中有）：从该场深度分析摘要的「亚盘」「大小球」行读取是否含「泊松 EV」及「与泊松EV一致/分歧」。结合赛果注明：当时 EV 指向哪边、推荐跟了哪边、结果如何；若推荐跟了盘赔而 EV 指向另一边且未中，或跟了 EV 方向命中，可纳入本场回顾与步骤 4c 教训提炼（如「亚盘与泊松 EV 分歧时跟盘未中，可长期对比跟盘 vs 跟泊松」），便于复盘时利用 EV 维度。
- **大小球盘口锚定（A 方案）**（若摘要中有）：从该场深度分析摘要的「泊松完整概率表」头两行读取 `大小球盘口锚定 = 启用/未启用（w=0.25, line=..., λtot: X→Y）`。逐场报告中必须带一行，便于长期评估锚定是否在“帮忙还是拖后腿”：
...
- 大小球盘口锚定（A 方案）效果统计（仅统计有记录的场次；旧摘要缺字段的不计入）：
  ...
  - **大小球 CLV 对比（如有）**：启用组均值 {+/-X.X%} vs 未启用组均值 {+/-X.X%}
