...
- **参数（第一版）**：
  - 锚定权重 \(w\)=**0.25**
  - 市场目标大球概率 \(p\_{mkt}\)=**0.50**（第一版不折算水位；后续如需更精确再用水位推隐含概率）
- **盘口处理（第一版，四分之一盘近似）**：将大小球盘口 line 映射到最近的 0.5 盘：
  - 2/2.5（2.25）→ 2.5
  - 2.5/3（2.75）→ 3.0
  - 其余半球盘（2.5、3.5）不变；整数盘（3.0）保持 3.0
...
3. **诱买操作**：引导散户买某一结果，实际看好其他结果
   - **形态识别（强制）**：当出现 **“欧指与亚盘反向变动”** 且同时伴随 **“水位异常/阀值触发/盘水矛盾”**（如亚盘主水持续打穿 ≤0.80 或 ≥1.05、或升盘升水/降盘降水这类矛盾组合），优先判为 **诱买（诱热门方向）**，并在第 8 步与第 10 步将其列为高权重疑点/核心矛盾，信心度与串关适合度应下调。
   - 判断方法：欧指与亚盘交叉验证（步骤 1.5 已抓亚盘时，本步对比欧指结论与亚盘初/即方向：一致则信号更可靠；矛盾且命中上述形态则按“诱买”处理）
   - 例：欧指降主胜（引导买主胜）但亚盘不升盘甚至降盘 + 主水持续走高/打穿阀值 → 典型诱买主胜信号
...
#### 泊松完整概率表（复盘用）
- λ1（主队）= {值}, λ2（客队）= {值}, 低比分轻量修正 = {启用/未启用}, 大小球盘口锚定 = {启用/未启用（w=0.25, line=2.25→2.5 或 2.75→3.0, λtot: X.XX→Y.YY）}
