...
- **此场已在当日 memory 有分析记录时**：在步骤 2 历史记忆召回后，若发现该场在当日 `## 深度分析` 中已有完整摘要，**必须通过 messaging 询问龙王**如何处理，不得自动跳过。消息示例：
  ...
  龙王选 2 或说了「重新分析」「刷新」→ 执行完整流程（包含步骤 1 + 1.5 数据抓取 + 10 步分析）。
...
4. **当日已分析过的场次（方式 2/3/4）**  
   ...
   - **若未说** → **先按方式 4 规则通过 messaging 询问龙王**（1. 看之前的分析报告 2. 重新分析）。龙王选 1 → 跳过步骤 1、1.5 及步骤 3（10 步分析），直接基于已有摘要推送并 announce；选 2 或回复「重新分析」「刷新」→ 执行全流程。
...
- **必抓 4 家**：澳彩、皇冠、365、易胜博。**先对 4 家分别分析并交叉认证**。
- 亚洲联赛：可选加入香港马会；若抓了马会，参与轨迹表与一致性判断。
...
综合前 9 步，提炼出本场比赛的核心判断要素。本步**直接引用或归纳**前 7 步已标出的内容：第 1～2 步（基本面/伤停）的关键信息与不确定处；第 5～7 步的 RLM、诱买/诱盘、某家与其余共识不一致、错盘形态；第 8 步的**具体矛盾项**与诱盘/RLM 摘要、价值评估方向相反；第 7 步的综合结论（三线一致或错位）。本步输出的矛盾与疑点将直接影响输出 1（综合评估）的**信心度**与**串关适合度**；矛盾多或疑点重则信心下调、慎串。
...
- **矛盾**：各维度或各公司之间的冲突信号。**引用第 8 步已列出的具体矛盾项**（如欧指 vs 亚盘、某维与另两维共识不符），可按需要合并或精简为 1～2 句，不新发明未在第 8 步列出的矛盾。**若第 4 步完成后发现**大小球结论与泊松整体 Top 6 方向相反（大小球 vs 泊松比分），该矛盾在**输出 1 综合评估**的「核心矛盾」中补入（因第 8 步执行时第 4 步尚未完成）。若无则写「各维度方向一致，无明显矛盾」。
