...
- **同时最多 3 个标签页**（避免浏览器内存压力和 titan007 限流）
...
- 先看 4 家（澳彩、皇冠、365、易胜博）是否同盘口、同方向 → 共识强，再以平博方向为参照。
...
- 先看 4 家（澳彩、皇冠、365、易胜博）是否同盘口同方向 → 共识强，再以平博方向为参照。
...
基于 9 步分析 + 比分概率建模，汇总给出最终结论。**推送和 memory 摘要均需包含分析时间**，格式为 `YYYY-MM-DD HH:mm`（如 2026-03-15 10:30）。
...
- **大小球**：基于进球数分析，给出盘口和方向，附带泊松预期总进球数。赔率依据从第 5 步进球数分析中提炼。方向不明确时标注「⚠️ 观望」
- **比分**：泊松模型 Top 3（最多 4 个），标注价值偏差
- **关键点 / 疑点 / 核心矛盾**：直接引用第 8 步分析的结论，是整个分析的精华浓缩。关键点和疑点至少各 1 条，核心矛盾如果没有可标注「无明显矛盾」
...
- 如果同一场比赛重新分析（龙王要求刷新） → **替换**该场比赛的子标题内容（通过匹配 `### {联赛} {主队} vs {客队}` 定位），不影响其他场次
...
- 让球：{主队让X球 主胜/...}（盘口：{X}）｜赔率依据：{亚盘关键发现}｜或 ⚠️ 观望（原因）
- 大小球：{大X.5球/小X.5球}（盘口：{X.5}，预期总进球：{λ1+λ2}）｜赔率依据：{大小球关键发现}｜或 ⚠️ 观望（原因）
