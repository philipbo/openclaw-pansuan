...
向钱冲直接提供比赛 ID，例如：「分析 2950979」「帮我看看 2950979 和 2784768」

→ 跳过初筛，直接用比赛 ID 拼接分析页 URL：`https://zq.titan007.com/analysis/{matchId}cn.htm`

### 方式 3：向钱冲提供分析页 URL

向钱冲直接粘贴 URL，例如：「分析这场 https://zq.titan007.com/analysis/2950979cn.htm」
...
- 如果指定的比赛已经在当日 memory 中有分析记录，提示向钱冲「此场已分析，是否重新分析？」，避免重复浏览器操作
...
每场比赛按以下 10 步依次分析（第 1-9 步为分析，第 10 步为比分建模），**完整展示分析过程**，让向钱冲能从中学习判断逻辑。
...
**这是向钱冲最核心的需求——比分预测。** 基于前 9 步分析的结论，用泊松分布计算比分概率排序。
...
内部仍计算完整概率分布（用于价值偏差分析和复盘追踪），但展示给向钱冲的只有 Top 3-4。
...
6. 输出 Top 3 比分（最多 4 个）供向钱冲选择串关组合

### 步骤 5：写入记忆

将分析结果追加写入 `memory/{今天日期}.md`。

**⚠️ memory 写摘要，不写全文**：完整 10 步分析已通过 messaging 推送给向钱冲，memory 只需存**结构化摘要**供复盘使用，避免 memory 文件膨胀到数千行。
...
- **分析场次超过 5 场**：分批处理，每场之间 browser 操作间隔 2 秒

## 输出

返回每场比赛的**完整分析过程 + 综合评估结论**，供 recommendation 推送给向钱冲。分析过程是推送消息的核心内容，不能省略。
