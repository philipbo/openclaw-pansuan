...
| 赛后复盘 | `skills/post-review/SKILL.md`     | 获取赛果→逐场核对→泊松模型评估→统计更新→复盘报告                                                   |
...
- `## 深度分析`：**结构化摘要**（基本面摘要 + 赔率摘要 + 综合评估 + 泊松比分简表，最多 6 个比分；不写完整 10 步分析全文——全文已通过 messaging 推送）
- 推荐内容（精选 + 候补 + 串关组合）：
  - 定时分析 → `## 推荐`
  - 手动 → `## 推荐（HH:MM）`
  - post-review 复盘时取**文件中最后一个**推荐 section 作为当日最终版。同一场若在当日多段推荐中出现，复盘只认最后一段内该场的推荐，不追溯更早段
...
- **统计数据**（累计战绩、联赛/玩法命中率、泊松、串关）：正常增长，只更新数字
